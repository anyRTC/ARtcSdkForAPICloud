...
/**
 * JavaScript for AR RTC SDK <br>
 * var module = api.require('ArRtc');<br>
 * module.xxx();
 */
public class ARRTCModule extends UZModule {
    ...
    private final HashMap<String, TextureView> mUidsList = new HashMap<>();
    private TextureView mLocalSurfaceView;

    public ARRTCModule(UZWebView webView) {
        super(webView);
    }
    ...
    private final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() {
                ...
                public void run() {
                    if (mJsCallbackLeaveChannel != null) {
                        ...
                        while (iterator.hasNext()) {
                            ...
                            surfaceView = (TextureView) entry.getValue();
                            removeViewFromCurWindow(surfaceView);
                            mRtcEngine.setupRemoteVideo(new VideoCanvas(null, Constants.RENDER_MODE_HIDDEN, (String) entry.getKey()));
        ...
        @Override
        public void onFirstRemoteVideoDecoded(final String uid, final int width, final int height, int elapsed) {
        ...
        @Override
        public void onUserOffline(final String uid, int reason) { // Tutorial Step 7
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
    ...
    public void jsmethod_joinChannel(final UZModuleContext moduleContext) {
        if (mRtcEngine != null) {
            ...
            int ret = mRtcEngine.joinChannel("", channelName, "", uid);
...
//          int result = mRtcEngine.setVideoProfile(w, h, fps, bitrate); // Earlier than 2.3.0
    ...
    public void jsmethod_setupLocalVideo(final UZModuleContext moduleContext) {
        if (mRtcEngine != null) {
            ...
            if (mLocalSurfaceView != null) {
                removeViewFromCurWindow(mLocalSurfaceView);
                int ret = mRtcEngine.setupLocalVideo(new VideoCanvas(null, Constants.RENDER_MODE_HIDDEN, ""));
                sendJsonResultToJs(moduleContext, ret);
                mLocalSurfaceView = null;
            }
            ...
            if (!moduleContext.isNull("rect")) {
                ...
                int ret = mRtcEngine.setupLocalVideo(new VideoCanvas(mLocalSurfaceView, rendermode, ""));
                sendJsonResultToJs(moduleContext, ret);
    ...
    public void jsmethod_setupRemoteVideo(final UZModuleContext moduleContext) {
        if (mRtcEngine != null) {
            ...
            String uid = moduleContext.optString("uid");
            TextureView remoteSurfaceView = null;

            if (moduleContext.isNull("rect")) {
                remoteSurfaceView = mUidsList.remove(uid);
                removeViewFromCurWindow(remoteSurfaceView);
                int ret = mRtcEngine.setupRemoteVideo(new VideoCanvas(null, Constants.RENDER_MODE_HIDDEN, uid));
                sendJsonResultToJs(moduleContext, ret);
    ...
    public void jsmethod_muteRemoteVideoStream(final UZModuleContext moduleContext) {
        if (mRtcEngine != null) {
            sendJsonResultToJs(moduleContext, mRtcEngine.muteRemoteVideoStream(moduleContext.optString("uid"), moduleContext.optBoolean("mute")));
        }
    }
...
//            sendJsonResultToJs(moduleContext, mRtcEngine.resumeAudio());
    ...
    public void jsmethod_muteRemoteAudioStream(final UZModuleContext moduleContext) {
        if (mRtcEngine != null) {
            sendJsonResultToJs(moduleContext, mRtcEngine.muteRemoteAudioStream(moduleContext.optString("uid"), moduleContext.optBoolean("mute")));
        }
    }
